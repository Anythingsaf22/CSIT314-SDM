...
@fundraising_activity_bp.route("/activities/stats", methods=["GET", "POST"])
@roles_required(FUNDRAISER)
def activity_stats():
    controller = view_activity_stats_controller()
    stats = None
    ...
    if request.method == "POST":
        ...
        if not activity_id:
            flash("Activity ID required.", "error")
            return render_template("activities/activity_stats.html")
        ...
        stats = controller.getStats(activity_id)

        if not stats:
            flash("Activity not found.", "error")

    return render_template("activities/activity_stats.html", stats=stats)

@fundraising_activity_bp.route("/activities/<int:activity_id>")
def view_single_activity(activity_id):
    controller = view_fundraising_activity_controller()

    activity = controller.viewActivityById(activity_id)
    ...
    return render_template(
        "activities/view_single_activity.html",
        activity=activity
    )
